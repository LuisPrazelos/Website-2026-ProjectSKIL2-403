    ...
    <div class="flex items-center gap-3">
        <form action="{{ route('cart.add', $surplus->id) }}" method="POST" class="flex items-center gap-3 w-full">
            ...
            <div class="flex items-center gap-2">
                <label for="quantity_{{ $surplus->id }}" class="text-sm text-gray-600">Hoeveelheid:</label>
                <input
                    type="number"
                    ...
                    value="1"
                    class="w-16 px-2 py-1 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                >
            </div>

            {{-- Add to cart button --}}
            <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 flex items-center gap-2 ml-auto">
